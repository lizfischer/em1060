...
<!--  <SCRIPT language="Javascript1.2" src="../java/password.js" type="text/javascript"></SCRIPT> -->
		...
		<!-- Output first title as level 1 heading-->
    		...
    		<div id="masthead"> <img class="em-logo" src="../images/logos/titlepage.jpg" alt="The Production and Use of English Manuscripts 1060 to 1220"/> <div id="masthead_print"> Edited by Orietta Da Rold, Takako Kato, Mary Swan and Elaine Treharne<br/> (University of Leicester, 2010; last update 2013)<br/>  http://www.le.ac.uk/ee/em1060to1220, ISBN 095323195X </div> </div>     		<!-- from here Google thingy -->
      <div id="searchBox">
...
<script type="text/javascript" src="../java/brand.js"></script>
  ...
  <xsl:template match="tei:note[@type='Owen']">
    <span class="ET"> {{Owen!! - 
      <xsl:apply-templates/>
      }} </span>
  </xsl:template>
...
<!-- =============================================================== -->
  <xsl:template match="tei:msContents">
    ...
    <span class="itemHeading">Manuscript Items:</span>
    <ol>
...
<!-- ========================================================== -->
  <xsl:template match="tei:msContents/tei:summary">
    ...
    <span class="itemHeading"><xsl:call-template name="makeID"/>Summary:</span>
    <p>
      <xsl:apply-templates/>
    </p>
  </xsl:template>
...
<!-- to display the Cameron number, from here: -->
  ...
  <xsl:template match="tei:msItem/tei:title[node()][@type='ms']">
	...
	<!-- Locus within decoNote within msItem, enter ":" after the folio or page number ====================== -->
...
<!-- Where you wish to list few additions in one msItem, use <locus> as the heading, within the <list> for listing the additions -->
...
<xsl:template match="tei:collation//tei:p[@n='quires']">
  ...
  <!-- If the quire structure needs to be explained in several blocks, then use <date> as the heading for the quire structures, and use <list> for listing the quire structure. -->
...
<!-- HOVER THE MOUSE AND POPS UP:
	making a title attribute means if you hover the mouse, the attribute value is displayed as a tooltip -->
