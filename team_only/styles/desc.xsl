...
<!-- =========================================================== -->
  ...
  elements if you have nothing to put in them)  -->
	...
	<xsl:template match="tei:msItem/tei:title[@type='uniform']">
	    <p>
	      <span class="itemHeading">Title</span> (standard): 
	      <xsl:apply-templates/>
	    </p>
	</xsl:template>
	<xsl:template match="tei:msItem/tei:title[@type='supplied']">
	    <p>
	...
	<!-- Locus within decoNote within msItem, ":" after the folio or page number ====================== -->
	<xsl:template match="tei:msItem/tei:decoNote/tei:list/tei:item/tei:locus[node()]">
			<span class="small"><xsl:apply-templates/>: </span>
	</xsl:template>
		<!-- We no longer give short titles  -->
...
<!-- When you want to list a few additions in one msItem,
...
<xsl:template match="tei:supportDesc/tei:support[node()]">
 <p class="totheright">   
  ...
  <xsl:template match="tei:support/tei:p[node()]">
  <p class="totheright">
  	<xsl:apply-templates/>
  	</p>
  </xsl:template>
...
<!-- Object 6: Summary (N.B. Short Descriptions Only) -->
...
<!-- Condition -->
<xsl:template match="tei:condition[node()]">
    <p class="totheright">
...
<!-- ========================================================== -->
  ...
  <!-- General purpose tei phr to html paragraph: 
  	Use <phr> for places where <p> is not possible, but want to describe in multiple paragraphs. -->
