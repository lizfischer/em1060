...
<!--                                                                                                    -->
<!-- and use <p n="marker"> for divisions in the ms ================================== -->
<!-- =========================================================== -->
  <ol>
...
<!-- originally ONLY FOR SHORT VERSION. to make links to the titles of the item-->
  ...
  <xsl:template match="tei:msItem" mode="toc">
    ...
    <xsl:variable name="pos" select="position()"/>
    	...
    	<!-- For those descriptions with short titles under MS Items. 
    		<a href="{concat('#',$msID,'-',$elementName,'-',$pos)}">
    		<xsl:value-of select="tei:title[@type='short'][node()]"/> -
    		<xsl:value-of select="tei:locus"/></a>-->

    	<a href="{concat('#',$msID,'-',$elementName,'-',$pos)}"><xsl:value-of select="tei:title[@type='short'][node()]"/><xsl:value-of select="tei:locus"/></a><xsl:call-template name="punctuate-list"/>
  </xsl:template>
