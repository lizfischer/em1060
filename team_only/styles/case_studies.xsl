...
-->
  ...
  <xsl:template match="tei:msItem" mode="toc">
    ...
    <xsl:variable name="pos" select="position()"/>
    	...
    	<a href="{concat('#',$msID,'-',$elementName,'-',$pos)}"><xsl:value-of select="tei:title[@type='short'][node()]"/><xsl:text> </xsl:text><xsl:value-of select="tei:locus"/></a><xsl:call-template name="punctuate-list"/>
  </xsl:template>
